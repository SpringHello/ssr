    ...
    <div class="center">
      ...
      <div class="body">
        <div class="body-left">
          ...
          <Page :total="total" :page-size="pageInfo.pageSize" :current="pageInfo.currentPage"
...
<script type="text/ecmascript-6">
  ...
  export default{
    ...
    asyncData ({store, route, cookies}) {
      return store.dispatch('getArticlePage', {route, cookies})
    },
    ...
    beforeRouteUpdate (to, from, next) {
      ...
      this.$store.dispatch('getArticlePage', {route: to})
      next()
    },
    methods: {
      update(keywordVal){
        this.keywordVal = this.keywordVal == keywordVal ? '' : keywordVal
        axios.post('/ruicloud/article/getMoreArticle.do', {
          articleTypeId: this.$route.params.typeId,
          keywordVal: this.keywordVal,
          page: this.pageInfo.currentPage,
          pageSize: this.pageInfo.pageSize
        }).then(response => {
          this.$store.state.moreArticle = response.data.result
        })
      },
      pageUpdate(current){
    ...
    computed: {
      ...
      articleList(){
        return this.$store.state.moreArticle.data
      },
      total(){
        return this.$store.state.moreArticle.total
      },
      tags(){
